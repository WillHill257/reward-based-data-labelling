...
export default Vue.extend({
  ...
  methods: {
    ...
    onAccept() {
      ...
      var acceptJobJson = {
        user: "60a10a0e5db8938e4f6948db",
      };

      if (this.labellers.includes("60a10a0e5db8938e4f6948db")) {
        alert("You have already accepted this job!");
        return;
      }

      if (this.author == "60a10a0e5db8938e4f6948db") {
        alert("You cannot accept a job you have created");
        return;
      }

      const jobID = this.$props.jobID;
      const addLabellerUrl =
        "http://localhost:4000/api/job/addLabeller/" + jobID;
